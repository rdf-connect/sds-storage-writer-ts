...
export async function handleTimestampPath(id: string, streamId: string, path: string, timestampValue: string, memberId: string, mongo: Collection<MongoFragment>, maxSize: number) {
    ...
    if (smallerIndex) {
        if (smallerIndex.count < maxSize || timestampValue === smallerIndex.timeStamp) {
            await mongo.updateOne({ leaf: true, streamId, id, timeStamp: smallerIndex.timeStamp }, { $inc: { count: 1 }, $push: { members: memberId } });
        } else {
            const largerIndex: undefined | MongoFragment = (await mongo.find({ leaf: true, id, timeStamp: { $gt: timestampValue }, streamId }).sort({ timeStamp: -1 }).limit(1).toArray())[0];
            ...
            await mongo.updateOne({ leaf: true, streamId, id, timeStamp: smallerIndex.timeStamp, path }, { "$push": { relations: { type: RelationType.GreaterThanOrEqualTo, value: timestampValue, bucket: timestampValue } } })
